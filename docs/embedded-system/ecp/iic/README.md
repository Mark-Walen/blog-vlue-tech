...
$\text{I}^2\text{C}$ 接口由串行时钟线（SCL）和串行数据线（SDA）组成。SDA 和 SCL 线都必须通过上拉电阻连接到 VCC。上拉电阻的大小取决于 $\text{I}^2\text{C}$ 线上的电容量（有关详细信息，请参阅$\text{I}^2\text{C}$ 上拉电阻计算（[SLVA689](https://www.ti.com/lit/an/slva689/slva689.pdf?ts=1704303283417)））。**只有在总线处于空闲状态时，才能传输数据**。如果在STOP 条件后 SDA 和 SCL 线都为高电平，那么总线被视为空闲。
...
1. 主设备需要在总线上发送一个带有从设备地址的开始条件，以及设备地址的后一位（R/$\overline{\text{W}}$ 位）设置为 0，表示写入。
   一个 7 位地址是0x2D，添加一个等于 0 的写入位，它将变为 0x5A（先将地址帧左移一位，然后逻辑或上读写位）。
